...
// IMPORTANT: Must use SERVICE_ROLE_KEY for delete operations (bypasses RLS)
...
async function run() {
  console.log('=== REGENERACIÓN FINAL CON SERVICE_ROLE_KEY ===\n')
  ...
  console.log(`Block subjects: ${filteredBlockSubjects.length}`)
  ...
  currData?.forEach((row: any) => {
    ...
    if (SPECIAL_GROUP_IDS.includes(row.group_id)) return
  ...
  const context: RuleContext = {
    ...
    normalWorkloadSubjectIds: SPECIAL_GROUP_SUBJECT_IDS,
  ...
  console.log(`Total bloques: ${blocksToAssign.length}`)
  ...
  // Borrar con SERVICE_ROLE_KEY
  console.log('\nBorrando horario anterior...')
  const { error: delError } = await supabase.from('sch_schedule_slots').delete().neq('id', '00000000-0000-0000-0000-000000000000')
  if (delError) {
    console.error('Error al borrar:', delError)
  }

  const { count: afterDel } = await supabase.from('sch_schedule_slots').select('*', { count: 'exact', head: true })
  console.log(`Slots after delete: ${afterDel}`)
  ...
  for (let i = 0; i < result.schedule.length; i += BATCH) {
    ...
    if (error) {
      console.error(`Error al insertar lote ${i}-${i+BATCH}:`, error.message)
      break
    }
  ...
  console.log(`\n✅ Total slots en BD: ${count} (insertados: ${insertedTotal})`)
