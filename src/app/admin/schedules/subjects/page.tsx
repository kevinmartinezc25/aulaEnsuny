...
export default function SubjectsPage() {
  ...
  const [newSubIsWorkload, setNewSubIsWorkload] = useState(true)
  const [catalogFilter, setCatalogFilter] = useState<'all' | 'workload' | 'non_workload'>('all')
  ...
  const filteredCatalogSubjects = useMemo(() => {
    return subjects
      ...
      .filter(s => {
        if (catalogFilter === 'workload') return s.is_academic_workload !== false
        if (catalogFilter === 'non_workload') return s.is_academic_workload === false
        return true
      })
  }, [subjects, searchSubject, catalogFilter])
  ...
  const handleSaveSubject = async () => {
    ...
    const fullPayload = {
      ...
      color: newSubColor,
      room_type: newSubRoomType,
      is_academic_workload: newSubIsWorkload
    }
    ...
    if (editingSubjectId) {
      ...
      if (error && error.code === '42703') {
        // Fallback si la migración de base de datos no se ha ejecutado aún
        const fallbackPayload = { name: newSubName.trim(), color: newSubColor, room_type: newSubRoomType }
        const retry = await supabase.from('sch_subjects').update(fallbackPayload).eq('id', editingSubjectId).select()
        data = retry.data
        error = retry.error
        if (!error) {
          toast.warning("Materia actualizada. Recuerda ejecutar horarios_migration_09_subject_academic_workload.sql en Supabase para persistir el atributo de Carga Académica.")
        }
      }

      if (!error && data) {
        setSubjects(subjects.map(s => s.id === editingSubjectId ? { ...s, ...data[0], is_academic_workload: newSubIsWorkload } : s).sort((a, b) => a.name.localeCompare(b.name)))
        resetSubjectForm()
        toast.success("Materia actualizada")
      } else if (error) {
        toast.error("Error al actualizar la materia: " + error.message)
      }
    } else {
      let { data, error } = await supabase
        .from('sch_subjects')
      ...
      if (error && error.code === '42703') {
        // Fallback si la migración de base de datos no se ha ejecutado aún
        const fallbackPayload = { name: newSubName.trim(), color: newSubColor, room_type: newSubRoomType }
        const retry = await supabase.from('sch_subjects').insert([fallbackPayload]).select()
        data = retry.data
        error = retry.error
        if (!error) {
          toast.warning("Materia creada. Recuerda ejecutar horarios_migration_09_subject_academic_workload.sql en Supabase para persistir el atributo de Carga Académica.")
        }
      }

      if (!error && data) {
        setSubjects([...subjects, { ...data[0], is_academic_workload: newSubIsWorkload }].sort((a, b) => a.name.localeCompare(b.name)))
        resetSubjectForm()
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
              <div className="flex items-center gap-3 flex-wrap flex-1">
                ...
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl text-xs font-semibold">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg transition-colors ${catalogFilter === 'all' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                  >
                    Todas ({subjects.length})
                  </button>
                  <button
                    onClick={() => setCatalogFilter('workload')}
                    className={`px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 ${catalogFilter === 'workload' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                  >
                    <GraduationCap className="h-3.5 w-3.5" />
                    Lectivas ({subjects.filter(s => s.is_academic_workload !== false).length})
                  </button>
                  <button
                    onClick={() => setCatalogFilter('non_workload')}
                    className={`px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 ${catalogFilter === 'non_workload' ? 'bg-white dark:bg-slate-700 text-purple-600 dark:text-purple-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                  >
                    <Users className="h-3.5 w-3.5" />
                    Núcleos / Inst. ({subjects.filter(s => s.is_academic_workload === false).length})
                  </button>
                </div>
              </div>
              ...
              {!isAddingSubject && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-xl transition-colors shadow-sm self-start sm:self-auto"
                >
            ...
            <AnimatePresence>
              {isAddingSubject && (
                ...
                >
                  ...
                  {/* Selector de Objeto de Carga Académica */}
                  <div className="mt-4 pt-3 border-t border-indigo-100 dark:border-indigo-800/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/70 dark:bg-slate-800/70 p-3.5 rounded-xl border border-indigo-100/70 dark:border-indigo-900/30">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-200">
                          ¿Es objeto de carga académica lectiva?
                        </span>
                        <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                          ...
                          {newSubIsWorkload ? 'Sí (Carga Lectiva)' : 'No (Núcleo / Institucional)'}
                        </span>
                      </div>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400">
                        {newSubIsWorkload
                          ? 'Las horas asignadas a docentes sumarán a su carga horaria lectiva obligatoria frente a grupo.'
                          : 'Materia para núcleos de área, comités o reuniones. Se programa en horarios pero NO suma a las horas lectivas del docente.'}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setNewSubIsWorkload(!newSubIsWorkload)}
                      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
                        newSubIsWorkload ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-700'
                      }`}
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          newSubIsWorkload ? 'translate-x-5' : 'translate-x-0'
                        }`}
                      />
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {filteredCatalogSubjects.map(subject => (
                    <div key={subject.id} className="group p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl flex items-center justify-between hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="overflow-hidden space-y-1">
                          ...
                          <div className="flex items-center gap-1.5 flex-wrap">
                            {subject.is_academic_workload === false ? (
                              <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300 border border-purple-200 dark:border-purple-800/60">
                                <Users className="h-3 w-3" />
                                Núcleo / Inst.
                              </span>
                            ) : (
                              <span className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60">
                                <GraduationCap className="h-3 w-3 text-indigo-500" />
                                Carga Lectiva
                              </span>
                            )}
                            {subject.room_type && subject.room_type !== 'aula' && (
                              <span className="text-[10px] font-semibold text-amber-600 dark:text-amber-400 flex items-center gap-1 bg-amber-50 dark:bg-amber-950/40 px-1.5 py-0.5 rounded border border-amber-200 dark:border-amber-800/40">
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                        <button onClick={() => { setEditingSubjectId(subject.id); setNewSubName(subject.name); setNewSubColor(subject.color); setNewSubRoomType(subject.room_type || 'aula'); setNewSubIsWorkload(subject.is_academic_workload !== false); setIsAddingSubject(true); }} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-md transition-colors" title="Editar Materia">
                          <Edit2 className="h-4 w-4" />
                        </button>
                        <button onClick={() => handleDeleteSubject(subject.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-md transition-colors" title="Eliminar Materia">
                  ...
                  {filteredCatalogSubjects.length === 0 && (
                    <div className="col-span-full py-8 text-center text-slate-400 text-sm italic">
                      No se encontraron materias con el filtro actual.
                    </div>
                  )}
                </div>
              )}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => {
                          ...
                          const chosenSub = subjects.find(s => s.id === val)
                          if (chosenSub) {
                            setIsOfficialWorkload(chosenSub.is_academic_workload !== false)
                          }
                        }}
